...
type Contract = { writes?: string[]; forbidsWrites?: string[] };
...
test('C-04 phase-writes-lock — historical faz bağlamı aktif fazdan bağımsız BLOCK eder', () => { const check = runPreflight({ site: 'excelarsiv', files: ['src/pages/index.astro'], contractOverride: 'faz-00' }).find((item) => item.id === 'P-03'); assert.equal(check?.status, 'FAIL'); const allowed = runPreflight({ site: 'excelarsiv', files: ['src/pages/index.astro'], contractOverride: 'faz-04' }).find((item) => item.id === 'P-03'); assert.equal(allowed?.status, 'PASS'); });
...
test('C-08 registry-single-writer — registry yazma yetkisi yalnız Faz 1 sözleşmesinde', () => { const contracts = readJson<Record<string, Contract>>('PHASE_CONTRACTS.json'); const path = 'data/seo/registry/excelarsiv_seo_registry.json'; const allowed = Object.entries(contracts).filter(([, contract]) => matchAny(path, contract.writes ?? []) && !matchAny(path, contract.forbidsWrites ?? [])).map(([phase]) => phase); assert.deepEqual(allowed, ['faz-01']); });
