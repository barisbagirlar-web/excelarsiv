...
test('C-04 phase-writes-lock — aktif faz runtime dışıysa runtime dosyasına yazmayı BLOCK eder', () => { const check = runPreflight({ site: 'excelarsiv', files: ['src/pages/index.astro'] }).find((item) => item.id === 'P-03'); assert.equal(check?.status, 'FAIL'); });
